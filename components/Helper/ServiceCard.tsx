...
const ServiceCard = ({ num, title }: Props) => {
  ...
  return (
    <div className="bg-black custom__service z-[100] relative transform rounded-2xl text-center p-6 shadow-md">
      <IconComponent className="w-[5rem] relative z-[1] mx-auto h-5[rem] text-green-400" />
      ...
      <p className="text-white text-[15px] opacity-60 mt-[0.8rem] ">
        ...
        minima nobis facilis? Porro, necessitatibus dolore. Deserunt sint in
        doloribus.
      </p>
      <p className="text-white text-[1.6rem] bfont-bold absolute top-3 right-4">{num}</p>
    </div>
  );
};
